                ...
                <div class="card-body">
                    <div style="text-align: end; padding: 1em">
                        ...
                        <button type="button" class="btn btn-primary"
                            onclick="window.location='{{ route('admin.lecturers.create') }}'">Thêm giảng viên</button>
                    </div>
                            ...
                            @foreach ($data as $each)
                                <tr>
                                    ...
                                    <td class="table-action">
                                        <a href="{{ route('admin.lecturers.edit', ['id' => $each->id]) }}"
                                            class="action-icon"> <i class="mdi mdi-pencil"></i></a>
                                    </td>
                                    <td class="table-action">
                                        <form id="form_delete_<?php echo $each->id; ?>"
                                            action="{{ route('admin.lecturers.destroy', ['id' => $each->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
    ...
    <script>
        $(document).ready(function() {
            $("#csv").change(function(e) {
                ...
                $.ajax({
                    url: '{{ route('admin.lecturers.import_csv') }}',
                    type: 'POST',
                    dataType: 'json',
