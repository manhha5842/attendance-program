...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('admin.lecturers.update', ['id' => $each->id]) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select" name="department_id">
                                ...
                                @foreach ($deparments_data as $option)
                                    <option value="{{ $option->id }}"
                                        @if ($each->department_id == $option->id) {{ 'selected' }} @endif>
                                        {{ $option->name }}
                                    </option>
                                @endforeach
                            </select>
